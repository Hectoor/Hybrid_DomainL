...
class cityscapesDataSet(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, img_size=(1024, 512), norm=False, random_mirror=False, random_crop=False, ignore_label=255, set='val', datapath=None):
        self.root = root
        ...
        for name in self.img_ids:
            ...
            if self.datapath:
                lbl_file = osp.join(self.root, self.datapath + "/%s" % name.split('/')[1])
            else:
...
class cityscapesDataSetSYN(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, img_size=(1024, 512), norm=False, random_mirror=False, random_crop=False, ignore_label=255, set='val', datapath=None):
        self.root = root
        self.list_path = list_path
        self.ignore_label = ignore_label
        ...
        self.datapath = datapath
        self.void_classes = [0, 1, 2, 3, 4, 5,
                             6, 9, 10, 14, 15, 16, 18, 22, 27, 29, 30, 31, -1]
        self.valid_classes = [7, 8, 11, 12, 13, 17, 19,
                              20, 21, 23, 24, 25, 26, 28, 32, 33]
    ...
    def __getitem__(self, index):
        ...
        if self.norm:
            ...
            image = image - np.array([122.67892, 116.66877, 104.00699]) # 其实是有做处理的
        image = image.transpose((2, 0, 1)).astype(np.float32)
        return image.copy(), label.copy(), np.array(size), name
    ...
    def encode_segmap(self, mask):
        #Put all void classes to zero
        for _voidc in self.void_classes:
            mask[mask == _voidc] = self.ignore_label    # 如果是无效的label，就置为255
        for _validc in self.valid_classes:
            mask[mask == _validc] = self.class_map[_validc]
        return mask
